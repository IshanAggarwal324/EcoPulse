...
class InternalAuthResponseTests(unittest.TestCase):
    def test_health_paths_are_not_blocked(self):
        ...
        self.assertIsNone(internal_auth_response("/health/live", "secret", None))

    def test_missing_configured_key_returns_503(self):
        response = internal_auth_response("/forecast/", "", None)
        self.assertIsNotNone(response)
        self.assertEqual(response.status_code, 503)

    def test_wrong_key_returns_401(self):
        response = internal_auth_response("/forecast/", "secret", "wrong")
        self.assertIsNotNone(response)
        self.assertEqual(response.status_code, 401)

    def test_matching_key_allows_request(self):
        self.assertIsNone(internal_auth_response("/forecast/", "secret", "secret"))
